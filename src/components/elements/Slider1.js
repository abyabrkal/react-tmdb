...
const Slider = ({ nowshowing, clickable }) => {
    ...
    return (
        ...
        <div className="popular-articles">
        <header className="popular-header header-card white-underline-links">
            <h2 className="header-card-title">Popular <br />this <br />month</h2>
        </header>
        ...
        <div className="mini-card-grid">
        {
            nowshowing.slice(0,8).map(movie => {
                return (
                    clickable ? (
                        <article key={movie.id} className="mini-card module module-article article cursor-pointer clickable" style={bgNSImage(movie.poster_path)}>
                            <Link to={`/${movie.id}`}></Link>
                        </article> 
                    ) : (
                        <article key={movie.id} className="mini-card module module-article article cursor-pointer clickable" style={bgNSImage(movie.poster_path)}></article>
                    ) 
                )
            })
        }
